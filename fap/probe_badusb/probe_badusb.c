/**
 * FL1PP3R69 v4.0 ARGUS VEIL — INKWELL probe sidecar
...
static bool write_sidecar(ProbeApp* app, const char* label) {
    ...
    snprintf(art, sizeof(art), "%s/artifacts/badusb", op_path);
    storage_common_mkdir(storage, art);
    ...
    snprintf(
        path,
        sizeof(path),
        "%s/badusb_%02u%02u%02u.meta.json",
        art,
...
static void draw_cb(Canvas* canvas, void* ctx) {
    ...
    canvas_draw_str(canvas, 2, 9, "INKWELL");
    canvas_set_font(canvas, FontSecondary);
    canvas_draw_str(canvas, 90, 9, "v" F69_VER);
    canvas_draw_line(canvas, 0, 11, 127, 11);
    const char* items[] = {"[1] Hash script", "[2] Auth gate", "[3] Run log", "[4] Lab only"};
...
static void input_cb(InputEvent* event, void* ctx) {
    ...
    if(event->key == InputKeyBack) {
        ...
        static const char* vals[] = {"hash_script", "auth_gate", "run_log", "lab_only"};
...
int32_t probe_badusb_main(void* p) {
    UNUSED(p);
