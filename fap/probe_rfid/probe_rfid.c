/**
 * FL1PP3R69 v4.0 ARGUS VEIL — LODGE probe sidecar
...
static bool write_sidecar(ProbeApp* app, const char* label) {
    ...
    snprintf(art, sizeof(art), "%s/artifacts/rfid", op_path);
    storage_common_mkdir(storage, art);
...
static void draw_cb(Canvas* canvas, void* ctx) {
    ...
    canvas_draw_str(canvas, 2, 9, "LODGE");
    canvas_set_font(canvas, FontSecondary);
    canvas_draw_str(canvas, 90, 9, "v" F69_VER);
    canvas_draw_line(canvas, 0, 11, 127, 11);
    const char* items[] = {"[1] EM4100 meta", "[2] HID meta", "[3] Indala meta", "[4] Generic LF"};
...
static void input_cb(InputEvent* event, void* ctx) {
    ...
    if(event->key == InputKeyBack) {
        ...
        static const char* vals[] = {"EM4100", "HID", "Indala", "generic_lf"};
...
int32_t probe_rfid_main(void* p) {
    UNUSED(p);
