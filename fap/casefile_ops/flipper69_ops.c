...
bool f69_write_active_op(Storage* storage, const F69Operation* op) {
    if(!op || !op->op_id[0]) return false;
    DateTime dt;
    furi_hal_rtc_get_datetime(&dt);
    char path[192];
    snprintf(path, sizeof(path), "%s/ACTIVE_OP.json", F69_ROOT);
    char body[384];
    snprintf(
        body,
        sizeof(body),
...
bool f69_clear_active_op(Storage* storage) {
    char path[192];
    snprintf(path, sizeof(path), "%s/ACTIVE_OP.json", F69_ROOT);
...
bool f69_write_checkpoint(Storage* storage, const F69Operation* op, const char* reason) {
    if(!op || !op->op_path[0]) return false;
    DateTime dt;
    furi_hal_rtc_get_datetime(&dt);
    char path[256];
    snprintf(path, sizeof(path), "%s/CHECKPOINT.json", op->op_path);
    const char* why = reason ? reason : "phase_advance";
    char body[320];
    snprintf(
        body,
        sizeof(body),
...
bool f69_try_resume_active(Storage* storage, F69Operation* op) {
    if(!op) return false;
    char path[192];
    snprintf(path, sizeof(path), "%s/ACTIVE_OP.json", F69_ROOT);
    File* file = storage_file_alloc(storage);
    ...
    char op_id[64] = {0};
    f69_parse_field(buf, "opId", op_id, sizeof(op_id));
    if(!op_id[0]) return false;

    char op_path[192];
    snprintf(op_path, sizeof(op_path), "%s/%s", F69_OPS_ROOT, op_id);
    if(!f69_op_load(storage, op, op_path)) return false;
    if(op->phase == F69PhaseClose) {
        f69_clear_active_op(storage);
        return false;
    }
    return true;
}
...
bool f69_op_create(Storage* storage, F69Operation* op, F69OpType type, F69PathNum path) {
    ...
    f69_write_active_op(storage, op);
    f69_write_checkpoint(storage, op, "create");
...
bool f69_op_advance_phase(Storage* storage, F69Operation* op) {
    ...
    f69_write_active_op(storage, op);
    f69_write_checkpoint(storage, op, "phase_advance");
    return true;
}
...
bool f69_op_close(Storage* storage, F69Operation* op) {
    ...
    f69_op_save(storage, op);
    f69_write_checkpoint(storage, op, "manual");
    f69_clear_active_op(storage);
    f69_update_index(storage);
    return true;
}
