/**
 * FL1PP3R69 v4.0 ARGUS VEIL — BITKEY probe sidecar
...
static bool write_sidecar(ProbeApp* app, const char* label) {
    ...
    snprintf(art, sizeof(art), "%s/artifacts/ibutton", op_path);
    storage_common_mkdir(storage, art);
    ...
    snprintf(
        path,
        sizeof(path),
        "%s/ibutton_%02u%02u%02u.meta.json",
        art,
...
static void draw_cb(Canvas* canvas, void* ctx) {
    ...
    canvas_draw_str(canvas, 2, 9, "BITKEY");
    canvas_set_font(canvas, FontSecondary);
    canvas_draw_str(canvas, 90, 9, "v" F69_VER);
    canvas_draw_line(canvas, 0, 11, 127, 11);
    const char* items[] = {"[1] DS1990A", "[2] Cyfral", "[3] Metakom", "[4] Generic"};
...
static void input_cb(InputEvent* event, void* ctx) {
    ...
    if(event->key == InputKeyBack) {
        ...
        static const char* vals[] = {"DS1990A", "Cyfral", "Metakom", "generic"};
...
int32_t probe_ibutton_main(void* p) {
    UNUSED(p);
