<#
.SYNOPSIS
    Fl1pp3r69 v2 desktop exfil bridge - USB serial + optional SD import.
.DESCRIPTION
...
function Import-SdTree {
    ...
    Get-ChildItem -Path $opsSrc -Directory | Where-Object { $_.Name -like "op-*" } | ForEach-Object {
        ...
        Write-TimelineEntry -OpPath $target -Event "sd_import" -Data @{ source = $SdRoot }
                ...
                if ($man.items) {
                    foreach ($item in $man.items) {
                        ...
                        if (Test-Sha256Match -Path $art -Expected $item.hash) {
                            ...
                            Write-TimelineEntry -OpPath $target -Event "hash_mismatch" -Data @{ path = $item.path }
                        }
                    }
                }
            } catch {
