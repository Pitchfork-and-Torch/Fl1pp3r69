...
Bump cache-bust query (`?v=`) on CSS/JS/images when shipping visual changes. Use a **date stamp** (e.g. `20260907`) for assets so cache-bust does not impersonate a product release. Keep OG `share-card.jpg` + JSON-LD `softwareVersion` aligned with the tagged product (currently **4.0.0 ARGUS VEIL**). User-facing copy is ASCII only.
